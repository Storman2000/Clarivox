    ...
    def __init__(self):
        self.intent_patterns = {
            IntentType.MEDICATION_REFILL: [
                r"refill.*prescription", r"ran out.*medication", r"need.*refill", 
                r"prescription.*refill", r"refill my", r"refill.*\w+\s*\d*mg",
                r"need more.*medication", r"out of.*medication"
            ],
            # RESCHEDULE must come BEFORE SCHEDULE to match "reschedule" first
            IntentType.APPOINTMENT_RESCHEDULE: [
                r"reschedule", r"change.*appointment", r"move.*appointment",
                r"different.*time", r"different.*date"
            ],
            IntentType.APPOINTMENT_SCHEDULE: [
                r"schedule.*appointment", r"book.*visit", r"make.*appointment", 
                r"set up.*appointment", r"new appointment"
            ],
